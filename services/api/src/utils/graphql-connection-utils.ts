import { MaybePromise, clamp } from '@ephys/fox-forge';
import type { FindByCursorResult } from '@ephys/sequelize-cursor-pagination';
import type { Type } from '@nestjs/common';
import { Model } from '@sequelize/core';
import { ArgsType, Field, Int, ObjectType } from '../esm-compat/nest-graphql-esm.js';
...
export function isNormalizedCursorPagination(arg: any): arg is INormalizedCursorPagination {
  return isCursorPagination(arg);
}
    ...
    get pageInfo(): PageInfo {
      ...
      return {
        ...
        endCursor: cursorResultPromise.then(res => {
          const lastNode = res.nodes.at(-1);

          return lastNode != null ? getCursor(lastNode, res) : null;
        }),
        get hasNextPage(): MaybePromise<boolean> {
          return cursorResultPromise.then(async result => result.hasNextPage());
        },
        get hasPreviousPage(): MaybePromise<boolean> {
          return cursorResultPromise.then(async result => result.hasPreviousPage());
        },
      };
    },
  };
