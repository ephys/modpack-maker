...
`,
  ...
  async searchJars(
    ...
    @Args('query', { type: () => String, defaultValue: '' }) luceneQuery: string,
    ...
    return sequelizeCursorToConnection(
      async () => this.modService.getProjectJars(project, { luceneQuery, pagination }),
      {
        totalCount: async () => this.modService.countProjectJars(project, { luceneQuery }),
      },
    );
  }
